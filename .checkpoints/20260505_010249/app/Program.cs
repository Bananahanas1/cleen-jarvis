...
public sealed class JarvisForm : Form
{
    ...
    private const string OllamaModel = "qwen2.5-coder:1.5b";
    ...
    private static string PendingTerminalCommand = "";
    private static string PendingTerminalWorkingDirectory = "";
    ...
    {
        return
            "Kommandon, du får stava lite fel:\n" +
            "- hjälp\n" +
            "- status\n" +
            "- diskstatus\n" +
            "- cachestatus\n" +
    ...
    private static string ListProjectFilesCompact()
    {
        ...
        {
            ".md", ".txt", ".json", ".cs", ".html", ".css", ".js", ".ps1", ".csproj", ".vbs"
        };
    ...
    private static string CreateCheckpointTool()
    {
        try
        {
            ...
            var checkpointName = DateTime.Now.ToString("yyyyMMdd_HHmmss");
    ...
    private static string RestoreLatestCheckpointTool()
    {
        ...
        try
        {
            ...
            foreach (var checkpointFile in files)
            {
                try
                {
                    var relative = Path.GetRelativePath(latest, checkpointFile);
    ...
    private static string WriteProjectFileTool(string input, bool append)
    {
        ...
        try
        {
            ...
            if (append)
                File.AppendAllText(fullPath, Environment.NewLine + text + Environment.NewLine);
            else
                File.WriteAllText(fullPath, text + Environment.NewLine);

            return append
                ? "Text tillagd i fil: " + fullPath
                : "Fil skriven: " + fullPath;
        }
        catch (Exception ex)
        {
            return "Kunde inte skriva filen: " + ex.Message;
        }
    }
    ...
    private static bool TryGetSafeProjectFilePath(string relativePath, bool mustBeExisting, out string fullPath, out string error)
    {
        ...
        {
            error = "Blockerat: tillåten filtyp är bara .md, .txt, .json, .cs, .html, .css, .js eller .ps1";
            return false;
        }
    ...
    private static string TerminalPreviewTool(string input)
    {
        ...
        PendingTerminalWorkingDirectory = workingDir;
    ...
    {
        if (string.IsNullOrWhiteSpace(PendingTerminalCommand))
            return "Det finns inget terminalkommando att köra. Skriv först: terminal preview: dotnet build";

        var command = PendingTerminalCommand;
        var workingDir = PendingTerminalWorkingDirectory;
        ...
        try
        {
            ...
            if (!process.WaitForExit(30000))
            {
                ...
                return "Kommandot tog över 30 sekunder och stoppades.";
            }
            ...
            return
                ...
                "Output:\n" + TrimTerminalOutput(output) + "\n\n" +
                "Error:\n" + TrimTerminalOutput(error);
        }
        catch (Exception ex)
        {
    ...
    private static string TerminalCancelRun()
    {
        PendingTerminalCommand = "";
        PendingTerminalWorkingDirectory = "";
        return "Terminalkommandot avbröts.";
    }
