...
public sealed class JarvisForm : Form
{
    ...
    // Sekundära fönster — skapas vid första användning, hålls vid liv mellan open/close.
    // Static eftersom JarvisForm är de facto en singleton och HandleMessageAsync är statisk.
    // Se docs\MULTI_WINDOW_DESIGN.md.
    private static BrainWindow? _brainWindow;
    private static FileExplorerWindow? _fileExplorerWindow;
    ...
    // När main-fönstret stängs ska sekundära fönster stängas tvingat så processen avslutas rent.
    // NeuroLinked Python-server stoppas också så vi inte läcker python.exe i bakgrunden.
    private void OnMainFormClosing(object? sender, FormClosingEventArgs e)
    {
        try { _brainWindow?.ForceClose(); } catch { /* best-effort cleanup */ }
        try { _fileExplorerWindow?.ForceClose(); } catch { }
        try { _neuroLinkedBridge.StopAsync().GetAwaiter().GetResult(); } catch { }
    }

    // Öppna eller fokusera Brain-fönstret. Skapas första gången, återanvänds sedan.
    public static string OpenBrainWindow()
    {
        try
        {
            if (_brainWindow is null || _brainWindow.IsDisposed)
                _brainWindow = new BrainWindow();
            _brainWindow.ShowOrFocus();
            return "Brain-fönstret öppnat. Stäng X för att dölja det (det fortsätter köra i bakgrunden).";
        }
        catch (Exception ex)
        {
            return "Kunde inte öppna Brain-fönstret: " + ex.Message;
        }
    }

    public static string OpenFileExplorerWindow()
    {
        try
        {
            if (_fileExplorerWindow is null || _fileExplorerWindow.IsDisposed)
                _fileExplorerWindow = new FileExplorerWindow();
            _fileExplorerWindow.ShowOrFocus();
            return "File Explorer-fönstret öppnat.";
        }
        catch (Exception ex)
        {
            return "Kunde inte öppna File Explorer: " + ex.Message;
        }
    }
    ...
    private static async Task<string> HandleMessageAsync(string input)
    {
        ...
        if (routedV1.Intent == CommandIntent.FileExplorerOpen)
        ...
        // BRAIN_AND_EXPLORER_OPEN_CHECK: secondary windows can be opened by name.
        if (CommandIs(command, "brain", "öppna brain", "oppna brain", "visa brain", "hjärna", "hjarna", "öppna hjärnan", "oppna hjarnan"))
            return OpenBrainWindow();

        if (CommandIs(command, "explorer", "filexplorer", "fil explorer", "öppna explorer", "oppna explorer", "öppna filexplorer", "oppna filexplorer"))
            return OpenFileExplorerWindow();
    ...
    private static string BuildHelp()
    {
        return string.Join(Environment.NewLine, new[]
        {
            ...
            "- /brain   (öppna 3D-fönstret)",
            "- /explorer   (öppna File Explorer-fönstret)",
    ...
    private static string SaveSmartMemory(string input, string type, int importance, string tags, int skipWords)
    {
        ...
        return $"Smart Memory sparat i memory.md. Type: {type}, Importance: {importance}/5.";
    }
        ...
        try
        {
            var payload = new
            {
                ...
                messages = new object[]
                {
                    ...
                    {
                        role = "system",
                        content = "Du är Jarvis. Svara kort, tydligt och på svenska. Du kör lokalt via Ollama. Här är relevant lokalt minne från memory.md:\n\n" + BuildMemoryContext()
                    },
                    new
                    {
                        role = "user",
