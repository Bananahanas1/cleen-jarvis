...
def handle_tool_call(name, arguments):
    ...
    if name == "read_brain":
        return make_request("/api/jarvis/summary")

    elif name == "brain_insights":
        return make_request("/api/jarvis/insights")

    elif name == "send_to_brain":
        obs_type = arguments.get("type", "text")
        content = arguments.get("content", "")
        return make_request("/api/jarvis/observe", "POST", {
            "type": obs_type,
            "content": content,
            "source": "claude"
        })
        ...
        return make_request("/api/jarvis/status")

    elif name == "brain_learned":
        fmt = arguments.get("format", "summary")
        if fmt == "detailed":
            return make_request("/api/jarvis/learned")
        return make_request("/api/jarvis/learned/summary")

    elif name == "recall_knowledge":
        query = arguments.get("query", "")
        limit = arguments.get("limit", 10)
        return make_request(f"/api/jarvis/recall?q={urllib.parse.quote(query)}&limit={limit}")

    elif name == "search_brain_memory":
        query = arguments.get("query", "")
        limit = arguments.get("limit", 20)
        return make_request(f"/api/jarvis/search?q={urllib.parse.quote(query)}&limit={limit}")
        ...
        if tags:
            data["tags"] = tags
        return make_request("/api/jarvis/remember", "POST", data)

    elif name == "brain_knowledge_stats":
        return make_request("/api/jarvis/knowledge")
        ...
        return make_request(f"/api/graphify/query?q={urllib.parse.quote(query)}&top_k={top_k}")

    elif name == "graphify_path":
        source = arguments.get("source", "")
        target = arguments.get("target", "")
        return make_request(
            f"/api/graphify/path?from={urllib.parse.quote(source)}&target={urllib.parse.quote(target)}"
        )

    elif name == "obsidian_search":
        query = arguments.get("query", "")
        top_k = arguments.get("top_k", 10)
        return make_request(f"/api/obsidian/search?q={urllib.parse.quote(query)}&top_k={top_k}")

    elif name == "obsidian_read":
        path = arguments.get("path", "")
        max_chars = arguments.get("max_chars", 20000)
        return make_request(f"/api/obsidian/note?path={urllib.parse.quote(path)}&max_chars={max_chars}")

    elif name == "obsidian_write":
        return make_request("/api/obsidian/write", "POST", arguments)
...
TOOLS = [
    ...
    {
        "name": "graphify_query",
        "description": "Search the project's Graphify knowledge graph for architecture context, files, components, and relationships.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Component, file, concept, or architecture phrase to search for"
                },
                "top_k": {
                    "type": "integer",
                    "description": "Maximum number of graph matches to return"
                }
            },
            "required": ["query"]
        }
    ...
    {
        "name": "graphify_path",
        "description": "Find a short path between two concepts in the project knowledge graph.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "source": {
                    "type": "string",
                    "description": "Starting node label, id, file, or concept"
                },
                "target": {
                    "type": "string",
                    "description": "Target node label, id, file, or concept"
                }
            },
    ...
    },
    {
        ...
        "description": "Search the shared Obsidian vault for durable project and memory notes.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Topic, component, or phrase to search for in Markdown notes"
                },
                "top_k": {
                    "type": "integer",
    ...
    {
        "name": "obsidian_read",
        "description": "Read one Markdown note from the Obsidian vault using a relative path returned by obsidian_search.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "path": {
                    "type": "string",
                    "description": "Relative path inside the Obsidian vault"
                },
                "max_chars": {
                    "type": "integer",
    ...
    {
        "name": "obsidian_write",
        "description": "Write or update a Markdown note in the shared Obsidian vault with explicit agent intent.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "path": {
                    "type": "string",
                    "description": "Relative note path inside the Obsidian vault"
                },
                "content": {
                    "type": "string",
