...
class Hippocampus(BrainRegion):
    ...
    def _sync_memory_strengths(self):
        """Keep restored memory patterns and replay weights aligned."""
        valid_memories = []
        valid_strengths = []
        for i, memory in enumerate(self.memory_buffer):
            pattern = np.asarray(memory, dtype=bool)
            if len(pattern) != self.n_neurons:
                continue

            strength = self._memory_strength[i] if i < len(self._memory_strength) else 1.0
            if not np.isfinite(strength) or strength <= 0:
                strength = 1.0

            valid_memories.append(pattern)
            valid_strengths.append(float(strength))

        self.memory_buffer = valid_memories
        self._memory_strength = valid_strengths

    def step(self, external_input: np.ndarray, t: float) -> np.ndarray:
        fired = super().step(external_input, t)
        rate = self.neurons.get_firing_rate()
        self._sync_memory_strengths()
