"""
NeuroLinked Agent â€” Plan â†’ Execute â†’ Verify â†’ Learn loop.
...
No LLM required at the core â€” you can drive it by handing it a pre-built
...
@dataclass
class AgentRun:
    """Full record of one agent execution â€” goal, plan, outcome."""
    run_id: str
    goal: str
    plan: List[PlanStep]
...
class Agent:
    ...
    def stats(self) -> dict:
        """Aggregate agent performance â€” feeds benchmarks."""
        if not self.history:
            return {
                "total_runs": 0,
                "success_rate": None,
...
# Helper â€” parse a plain-JSON plan into PlanSteps
