...
def save_brain(brain, path=None, force=False):
    ...
    for name, region in brain.regions.items():
        ...
        if name == "hippocampus" and hasattr(region, "memory_buffer"):
            if hasattr(region, "_sync_memory_strengths"):
                region._sync_memory_strengths()
            selected_memories = region.memory_buffer[-100:]
            selected_strengths = region._memory_strength[-len(selected_memories):] if selected_memories else []
            memories = [m.tolist() for m in selected_memories]  # Last 100
            reg_state["memory_buffer"] = memories
            reg_state["memory_strength"] = [float(s) for s in selected_strengths]
        ...
        if os.path.exists(reg_dir):
            for name, region in brain.regions.items():
                ...
                if name == "hippocampus" and "memory_buffer" in reg_state:
                    ...
                    region._memory_strength = [
                        float(s) for s in reg_state.get("memory_strength", [])
                    ]
                    if hasattr(region, "_sync_memory_strengths"):
                        region._sync_memory_strengths()
